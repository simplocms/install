...
export function modelViewChangeType( evt, data, conversionApi ) {
	...
	if (data.attributeNewValue !== 'bulleted' && data.attributeNewValue !== 'numbered') {
        viewWriter.setAttribute('class', data.attributeNewValue, viewList);
    } else {
 ...
 * @param {Array.<object>} listTypes Configured list types.
 * @param {module:utils/eventinfo~EventInfo} evt An object containing information about the fired event.
 * @param {Object} data An object containing conversion input and a placeholder for conversion output and possibly other values.
 * @param {Object} conversionApi Conversion interface to be used by the callback.
 */
export function viewModelConverter( listTypes, evt, data, conversionApi ) {
	if ( conversionApi.consumable.consume( data.viewItem, { name: true } ) ) {
		const writer = conversionApi.writer;
		const conversionStore = conversionApi.store;
        ...
        if (parent.name === 'ol') {
            type = 'numbered';
        } else if (listTypes && Array.isArray(listTypes) && listTypes.length && parent.getAttribute('class')) {
            const listClass = parent.getAttribute('class');

            if (listTypes.some(type => type.name === listClass)) {
                type = listClass;
            }
        }
...
function generateLiInUl( modelItem, conversionApi ) {
	...
	const viewWriter = conversionApi.writer;
	const listType = modelItem.getAttribute( 'listType' );
	const listName = listType === 'numbered' ? 'ol' : 'ul';
	const viewItem = createViewListItemElement( viewWriter );

	const viewList = viewWriter.createContainerElement( listName );

    if (listType !== 'bulleted' && listType !== 'numbered') {
        viewWriter.setAttribute('class', listType, viewList);
    } else {
        viewWriter.removeAttribute('class', viewList);
    }
