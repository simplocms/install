 ...
 */
export default class ListEditing extends ListEditingPlugin {
    /**
     * @inheritDoc
     */
    static get requires() {
    ...
    init() {
        ...
        const listTypes = editor.config.get('list.types');
        ...
        data.upcastDispatcher.on('element:li', ( evt, data, conversionApi ) => {
            viewModelConverter(listTypes, evt, data, conversionApi)
        });
        ...
        if (types && Array.isArray(types) && types.length) {
            editor.commands.add('bulletedList:default', new ListCommand(editor, 'bulleted'));
            types.forEach(option =>
                editor.commands.add(`bulletedList:${ option.name }`, new ListCommand(editor, option))
            );
        } else {
